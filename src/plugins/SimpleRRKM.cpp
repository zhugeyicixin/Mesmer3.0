...
namespace mesmer
{
	...
	public:
		...
		virtual bool calculateTestMicroCnlFlux(Reaction* pReact);
	...
	bool SimpleRRKM::calculateTestMicroCnlFlux(Reaction* pReact)
	{
		Molecule* pTS = pReact->get_TransitionState();

		ctest << "SimpleRRKM::calculateTestMircoCnlFlux\tTypeID:\t" << getID() << endl; 
		ctest << "Error! This method has been abandoned temporarily and should not be used!" << endl; 
		...
		//printf("simpleRRKM\treactionType:\t%s\n",pReact->getReactionType());
		//Molecule* pReact1 = pReact->get_reactant();
		//vector<double> ReactcellDos;
		//if(!pReact1->getDOS.getCellDensityOfStates(ReactcellDos))
		//{
		//	return false;
		//}	
		return true;
