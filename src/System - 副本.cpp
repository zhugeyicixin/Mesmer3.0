...
namespace mesmer
{
	...
	//System::System(): m_pMoleculeManager(NULL),
	//	m_pReactionManager(NULL),
	//	m_CalcMethod(NULL),
	//	m_pTitle(NULL),
	//	m_pDescription(NULL)
	//{
	...
	// return a pointer to a deepcopy of this system
	//System* System::deepCopy()
	//{
	//	System* sys_copy = new System(*this);
	//	sys_copy->m_pMoleculeManager = new MoleculeManager(*(this->m_pMoleculeManager));
	//	sys_copy->m_pReactionManager = new ReactionManager(*(this->m_pReactionManager));
	//	//sys_copy->m_CalcMethod = new CalcMethod(*(this->m_CalcMethod));
	//	if (this->m_pTitle)
	//	{
	//		sys_copy->m_pTitle = new char[strlen(this->m_pTitle)];
	//		//strcpy(sys_copy->m_pTitle, this->m_pTitle);
	//	}
	//	if (this->m_pDescription)
	//	{
	//		sys_copy->m_pDescription = new char[strlen(this->m_pDescription)];
	//		//strcpy(sys_copy-m_pDescription, this->m_pDescription);
	//	}
	//	return sys_copy;
	//}
	...
	//
	void System::executeTestCalculation(int grainSize, double EAboveTop)
	{
		assert(m_CalcMethod);
		m_Env.GrainSize = grainSize;
		m_Env.EAboveHill = EAboveTop;
		ctest << "System::executeTestCalculation" << endl;
		m_CalcMethod->DoTestCalculation(this);
	}
	...
	// over all PT values, constant parameters
	bool System::testCalculate(double& chiSquare, vector<double> &residuals, bool writeReport)
	{
		// Controls the print-out of grain/cell DOS in each cycle (This is only for source term)
		if (m_Flags.cellDOSEnabled) m_Flags.cyclePrintCellDOS = true;
		if (m_Flags.grainDOSEnabled) m_Flags.cyclePrintGrainDOS = true;
		...
		for (calPoint = 0; calPoint < PandTs.size(); ++calPoint) {
			...
			// Build collison matrix for system.
			if (!m_collisionOperator.BuildTestReactionOperator(m_Env, m_Flags, writeReport))
				throw (std::runtime_error("Failed building system collison operator.")); 
