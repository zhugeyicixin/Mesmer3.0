#ifndef GUARD_Sobol_h
#define GUARD_Sobol_h
...
// Date:   10/Aug/2013
//
// This header file contains the declaration of the Sobol class. This class wraps the
// original FORTRAN77 version by Bennett Fox and extends the C++ version by John Burkardt
//
//-------------------------------------------------------------------------------------------
...
static const size_t DIM_MAX2 = 1111 ;
static const size_t LOG_MAX  = 30 ;
...
private:
  ...
  template<class T> 
  void initializeV (T v[DIM_MAX2][LOG_MAX]);
...
//
//    This routine uses T for integers and U for real values.
//
...
//  Parameters:
//
//    Input, T DIM_NUM, the number of spatial dimensions.
//    DIM_NUM must satisfy 1 <= DIM_NUM <= 1111.
//
//    Input/output, T *SEED, the "seed" for the sequence.
...
//
//    Output, U QUASI[DIM_NUM], the next quasirandom vector.
//
{
  static T atmost;
  ...
  static T poly[DIM_MAX2] =
  {
    ...
    16381 };
  static U recipd;
  static T seed_save = - 1;
  T seed_temp;
  static T v[DIM_MAX2][LOG_MAX];
